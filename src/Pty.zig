...
const first_letter = [_]u8{ 'p', 'q', 'r', 's', 't', 'u', 'v', 'w' };

fn openPtyDarwin() !Pty {
    const p = try posix.open("/dev/ptmx", .{ .ACCMODE = .RDWR, .NOCTTY = true }, 0);
    errdefer posix.close(p);
    ...
    for (first_letter) |c| {
        for (0..16) |i| {
            const sname = try std.fmt.bufPrint(&buf, "/dev/tty{c}{x}", .{@as(u8, @truncate(c)), i});
            const t = posix.open(sname, .{ .ACCMODE = .RDWR, .NOCTTY = true }, 0) catch continue;
            if (t != -1) {
                log.info("pty: {d}, tty: {d} - {s}", .{p, t, sname});
                return .{
                    .pty = p,
                    .tty = t,
                };
